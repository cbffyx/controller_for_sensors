...
#define W25Qx_Enable() 			GPIO_WriteBit(CS_GPIO_Port, CS_Pin, Bit_RESET);delay_ms(5)
#define W25Qx_Disable() 		GPIO_WriteBit(CS_GPIO_Port, CS_Pin, Bit_SET);delay_ms(100)
...
void W25QxInit(void)
{
	...
	GPIO_InitStruct.GPIO_Pin = CS_Pin|SCLK_Pin|MOSI_Pin;
...
static uint8_t BSP_W25Qx_SPI_Transmit( uint8_t *pData, uint16_t Size )
{
	uint16_t i = 0;
...
static uint8_t BSP_W25Qx_SPI_Receive( uint8_t *pData, uint16_t Size )
{
	uint16_t i = 0;
	uint8_t status = W25Qx_OK;
...
static uint8_t BSP_W25Qx_WriteEnable(void)
{
	...
	while(BSP_W25Qx_GetStatus() == W25Qx_BUSY)
	{
		...
		for(i=0; i<W25Qx_TIMEOUT_VALUE/10; i++)
		{
			delay_ms(10);
		}
...
uint8_t BSP_W25Qx_Write(uint8_t* pData, uint32_t WriteAddr, uint32_t Size)
{
  ...
  do
  {
    ...
    if (BSP_W25Qx_SPI_Transmit(pData,current_size) != W25Qx_OK)
    {
      return W25Qx_ERROR;
    }
			W25Qx_Disable();
...
uint8_t BSP_W25Qx_Erase_Block(uint32_t Address)
{
	...
	{
		/* Check for the Timeout */
		for(i=0; i<W25Q128FV_SECTOR_ERASE_MAX_TIME/10; i++)
		{
			delay_ms(10);
		}
...
uint8_t BSP_W25Qx_Erase_Chip(void)
{
	...
	{
		/* Check for the Timeout */
		for(i=0; i<W25Q128FV_BULK_ERASE_MAX_TIME/10; i++)
		{
			delay_ms(10);
		}
